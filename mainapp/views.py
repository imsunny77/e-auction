...
def order(request):
    ...
    param_dict = {
                ...
                'MID': 'exFdzQ74573022297895', 
                'ORDER_ID': models.AddProduct.objects.get(), 
                'TXN_AMOUNT': models.AddProduct.objects.get(),
                'CUST_ID': '1',
    ...
    param_dict['CHECKSUMHASH'] = Checksum.generate_checksum(param_dict, 'kyokTQRzYqkMpika')
...
def handlerequest(request): #paytm will send POST request herre
    ...
    verify = Checksum.verify_checksum(response_dict, 'kyokTQRzYqkMpika', checksum)
    if verify:
